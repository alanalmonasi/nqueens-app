      ...
      <>
         ...
         <div style={chessBox}>
            {currentChessBoard.map((r, x) =>
               r.map((c, y) => <Cell queen={c} key={`${x}${y}`} color={x+y} />))} 
         </div>
         <div style={{display: size <= 7 ? 'none' : ''}} className="solutions">
            <button onClick={increment} className="btn btn-primary mt-4">Next solution</button>
            < hr />
            <div>Current solution: {JSON.stringify(currentChessBoard).split(',')}</div>
            <div aria-label="total-solutions">Total solutions for this board size: {chessBoard.length}</div>
            <div id="remaining-solutions">Remaining solutions: {chessBoard.length - 1 - nextResult}</div>
         </div>
      </>
   )
}